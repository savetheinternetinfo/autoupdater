...
let verifyRequest = function(req, secret){
    ...
    if (!req.headers["x-github-delivery"]){
        ...
        (response.errors).push("No id found in the request");
    }

    if (!req.headers["x-github-event"]){
        response.valid = false;
        (response.errors).push("No event found in the request");
    }

    let sign = req.headers["x-hub-signature"] || "";
    if (!sign){
        response.valid = false;
        (response.errors).push("No signature found in the request");
    }

    let reqData = JSON.stringify(req.body);
    if (!verifySignature(secret, reqData, sign)){
        response.valid = false;
        (response.errors).push("Failed to verify signature");
    }
